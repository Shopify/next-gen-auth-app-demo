...
> **Important:** This guide assumes your app has the beta `app_session_token`
...
See the next session for a guide on setting up your `.env` file.

You will need to use [ngrok][3] to make your localhost port 3000 available to
...
shopify.dev][4]. You can then skip ahead to the section [__Making authenticated
...
4. In the root directory of your app, create a `.env` file to specify your
...
* This sample includes `SHOPIFY_DOMAIN` to help
first-party app developers easily specify their local domains (e.g
`myshopify.com` and `myshopify.io`)
...
Before you can install your app on your store, there are a few more changes required.
...
* This lines makes sure that shops with the domain specified by
...
2. Use rails to generate a new component called `App.js`:
...
This section assumes that your app meets the following prerequisities:

* Your app has the beta feature `app_session_token` enabled
...
* This ensures that if your app is using JWT tokens, cookies are no longer used
to secure your app.
* `valid_jwt_header?` is a temporary check to include within your app. This
check moved to the `shopify_app` library in future releases.

3. If you are trying to protect a resource in your routes file, you can do so
by extending that resource to be of class `AuthenticatedController`. In this example, we are protecting our `graphql` endpoint, so we made the
following change in `app/controllers/graphql_controller.rb`:
...
If your dashboard returns the following text:
